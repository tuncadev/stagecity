...
/**
 * Class for declaring the content importer used in the Civi Framework plugin
 *
 * @package Civi_Framework
 */

/**
 * Class Civi_Content_Importer
 */
class Civi_Content_Importer
{
	...
	/**
	 * The instance of the Civi_Import_Logger class.
	 *
	 * @var Civi_Import_Logger
	 */
	public $logger;
	...
	public function __construct($importer_options = array(), $logger = null)
	{
		...
		// Get the Civi_Importer instance.
		$this->tmi = Civi_Importer::instance();
	}
	...
	public function import_content($import_file_path)
	{
		...
		set_time_limit(apply_filters('civi_time_limit_for_demo_data_import', 300));
		...
		if (!apply_filters('civi_regenerate_thumbnails', false)) {
	...
	public function new_ajax_request_maybe($data)
	{
		...
		if ($time > apply_filters('civi_time_for_one_ajax_call', 25)) {
			$response = array(
				'status'  => 'newAJAX',
			...
			Civi_Import_Logger::append_to_file(
				esc_html__('New AJAX call!', 'civi-framework') . PHP_EOL . $message,
	...
	private function set_current_importer_data()
	{
		...
		set_transient('civi_importer_data', $data, 0.1 * HOUR_IN_SECONDS);
	}
}
